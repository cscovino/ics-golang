...
func (p *Parser) parseEvents(cal *Calendar, eventsData []string) {
	for _, eventData := range eventsData {
		...
		event.SetUID(p.parseEventUid(eventData))
...
func (p *Parser) parseEventUid(eventData string) string {
	re, _ := regexp.Compile(`UID(:|;)(.*?\r?\n)(\s.*?\r?\n)*`)
...
func (p *Parser) parseEventDescription(eventData string) string {
	re, _ := regexp.Compile(`DESCRIPTION.*:.*?\r?\n(?:\s+.*?\r?\n)*`)
	result := re.FindString(eventData)
	return trimField(strings.Replace(strings.Replace(result, "\r", "", -1), "\n", "", -1), "DESCRIPTION.*:")
}
